...
def create_sequece_dataloaders(dataset, seq_length, batch_size, device):
    # 使用对 data / next /Target 创建一个数据集（在这种情况下，数据是一个 seq_length 测量序列（18 个特征），
    # next 是序列中的下一个值，目标是具有测量值的下一个值（18 个特征））。 当您插入一个测量值时，模型应该会输出下一个测量值
...
def create_dataloaders(dataset, device):
    # 创建一个有data/target对的数据集（本例中数据是一个测量值（18个特征），目标是下一个测量值（18个特征））。
    # 当你插入一个测量值时，模型应该输出下一个测量值
    ...
    for i in models:
        if models[i][7] is True:
            ...
            if colab is True:
                ...
                files.download(path_save + '/Transformer_' + str(models[i][8]) + '.pt')
